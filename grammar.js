module.exports = grammar({
  ...
  rules: {
      ...
      using_index: ($) => seq(word('using'), word('index'), optional(word('seek')), $.variable, $.node_label, '(', $.property_key_name, repeat(seq(',', $.property_key_name)), ')'),
      using_scan: ($) => seq(word('using'), word('scan'), $.variable, $.node_label),
      using_join: ($) => seq(word('using'), word('join'), word('on'), $.variable, repeat(seq(',', $.variable))),
      ...
      foreach: ($) => seq(word('foreach'), '(', $.variable, word('in'), $.expression, '|', repeat1($.updating_clause), ')'),
      ...
      _show_entity: ($) => choice(
        ...
        seq(word('database'), $.symbolic_name),
        seq(word('default'), word('database')),
        seq(word('home'), word('database')),
      ),
      ...
      create_constraint: ($) => seq(word('create'), word('constraint'), optional($.symbolic_name), optional($._if_not_exists), choice(seq(word('for'), $._schema_pattern, word('require'), $._constraint_predicate), seq(word('on'), $._schema_pattern, word('assert'), $._constraint_predicate))),
      drop_constraint: ($) => seq(word('drop'), word('constraint'), $.symbolic_name, optional($._if_exists)),
      _schema_pattern: ($) => choice($.node_pattern, $.relationships_pattern),
      _constraint_predicate: ($) => seq(choice($._constraint_property, seq('(', $._constraint_property, repeat(seq(',', $._constraint_property)), ')')), choice(seq(word('is'), word('unique')), seq(word('is'), word('not'), word('null')), seq(word('is'), word('node'), word('key')))),
      _constraint_property: ($) => seq($.variable, repeat1($.property_lookup)),
      _if_not_exists: ($) => seq(word('if'), word('not'), word('exists')),
      _if_exists: ($) => seq(word('if'), word('exists')),
      ...
      anonymous_pattern_part: ($) => choice($.shortest_path_pattern, $.pattern_element),
      ...
      atom: ($) => choice($.literal, $.parameter, $.case_expression, seq(word('count'), /\(\s*\*\s*\)/), $.reduce_expression, $.extract_function, $.filter_function, $.map_projection, $.list_comprehension, $.pattern_comprehension, $.quantifier, $.shortest_path_pattern, $.pattern_predicate, $.parenthesized_expression, $.function_invocation, $.existential_subquery, prec.left($.variable)),
      reduce_expression: ($) => seq(word('reduce'), '(', $.variable, '=', $.expression, ',', $.id_in_coll, '|', $.expression, ')'),
      extract_function: ($) => seq(word('extract'), '(', $.filter_expression, optional(seq('|', $.expression)), ')'),
      filter_function: ($) => seq(word('filter'), '(', $.filter_expression, ')'),
      map_projection: ($) => seq($.variable, '{', optional(seq($.map_projection_element, repeat(seq(',', $.map_projection_element)))), '}'),
      map_projection_element: ($) => choice($.all_properties_selector, seq($.property_key_name, ':', $.expression), $.property_lookup, $.variable),
      all_properties_selector: () => token('.*'),
